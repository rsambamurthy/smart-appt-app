...
export const duesApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    initiatePayment: builder.mutation<{ data: { order_id: string; amount: number; key_id: string } }, { bill_id: string }>({ query: (body) => ({ url: '/dues/payments/initiate', method: 'POST', body }), invalidatesTags: ['Payment'] }),
